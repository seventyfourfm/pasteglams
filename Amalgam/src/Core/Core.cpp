...
static inline bool CheckDXLevel()
{
	...
	{
		const char* sMessage = "You are running with graphics options that pastegram does not support. It is recommended for -dxlevel to be at least 90.";
		SDK::Output("pastegram", sMessage, WARNING_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG, ICON_MD_WARNING);
	}
...
void CCore::AppendFailText(const char* sMessage)
{
	...
	{
		m_ssFailStream << "GO FUCK YOURSELF" "\n";
		m_ssFailStream << "\n";
	}
...
void CCore::LogFailText()
{
	try
	{
		...
		m_ssFailStream << "Logged to pastegram\\fail_log.txt. ";
	}
	catch (...) {}
...
void CCore::Unload()
{
	...
	SDK::Output("pastegram", "Unloaded", INFO_COLOR, OUTPUT_CONSOLE | OUTPUT_DEBUG);
}